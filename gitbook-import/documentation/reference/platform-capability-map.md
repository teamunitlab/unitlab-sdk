---
description: "Review the current July 2026 platform scope and the enterprise value of one multimodal operating model."
icon: list-check
layout:
  width: wide
---
...
{% hint style="info" %}
**Who this guide is for:** Technical evaluators, platform owners, procurement, and solution architects. **You will:** Map requirements to current product areas and identify the object, role, workflow, and delivery boundary for each capability.
{% endhint %}
...
{% stepper %}
{% step %}
### 1. List the required modalities, source systems, annotation structures, workflow states, quality controls, integrations, and outputs
{% endstep %}
{% step %}
### 2. Map each requirement to the current capability summary
{% endstep %}
{% step %}
### 3. Identify dependencies between data, grouping, ontology, workflow, model, role, and release configuration
{% endstep %}
{% step %}
### 4. Validate uncertain requirements in a representative pilot
{% endstep %}
{% step %}
### 5. Record supported behavior, current limits, owners, and acceptance evidence
{% endstep %}
{% endstepper %}
