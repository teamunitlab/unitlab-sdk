---
description: "Create frame-aware objects and tracks, operate the timeline, apply temporal properties, validate difficult motion, and submit controlled video work."
icon: video
layout:
  width: wide
---
...
{% hint style="info" %}
**Who this guide is for:** Video annotators, reviewers, project managers, ontology designers, and training leads. **You will:** Complete a video task with correct geometry, identity, temporal extent, dynamic state, and workflow outcome.
{% endhint %}

## Before you begin

- A processed video Data Unit and an active workflow task.
- A Live ontology whose geometry and properties support the required video labels.
- Instructions defining object presence, identity, occlusion, re-entry, scene cuts, interpolation, and dynamic-state policy.
- A reviewer calibration set covering slow, fast, crowded, occluded, and discontinuous motion.
...
{% stepper %}
{% step %}
### 1. Open the video task from the Task Queue and confirm the item, stage, and Instructions
{% endstep %}
{% step %}
### 2. Inspect playback, frame count, current frame, timeline zoom, loop, and View Settings
{% endstep %}
{% step %}
### 3. Choose the class and geometry and create a seed annotation on a reliable frame
{% endstep %}
{% step %}
### 4. Define the track’s active range and complete static properties
{% endstep %}
{% step %}
### 5. Propagate with full, forward, or backward Auto-Tracking, or add manual keyframes and interpolation
{% endstep %}
{% step %}
### 6. Create dynamic object or Item Properties over their exact temporal ranges
{% endstep %}
{% step %}
### 7. Inspect timeline rows, keyframes, hidden ranges, identities, and mask interpolation
{% endstep %}
{% step %}
### 8. Correct drift, missed frames, topology changes, occlusion, re-entry, and scene cuts
{% endstep %}
{% step %}
### 9. Save, confirm history and validation state, and submit through the active workflow action
{% endstep %}
{% step %}
### 10. Reviewer: inspect transition frames and either approve or reject with a precise reason and comment
{% endstep %}
{% endstepper %}
...
1. **Confirm temporal policy.** Define when an object begins and ends, how identity is preserved through occlusion, and how re-entry, scene cuts, blur, truncation, and camera changes are handled.
2. **Choose the seed frame.** Use a clear frame where the complete object is visible whenever tracking will be used.
3. **Create the first geometry.** Draw the correct class and geometry, then complete required static properties.
4. **Set the temporal range.** Use the timeline handles or default annotation length to establish the intended presence interval.
5. **Propagate intentionally.** Choose Auto-Tracking when visual evidence should drive predictions; choose interpolation when manually placed keyframes should determine in-between geometry.
6. **Add dynamic state.** Apply time-varying object or Item Properties to their exact ranges.
7. **Review the timeline.** Inspect keyframes, connected spans, hidden ranges, identity changes, and transitions at high timeline zoom.
8. **Review the canvas.** Check geometry at entry, exit, direction changes, occlusion, deformation, scale change, and scene cuts.
9. **Save and validate.** Confirm history, required values, and Invalid state before the stage action.
10. **Submit through the workflow.** Use Submit, Send to Review, Approve, or Reject according to the active stage; do not treat save as completion.

## Verify the result

- [ ] The object identity is stable across its intended temporal range.
- [ ] Entry, exit, occlusion, re-entry, scene cuts, and scale changes follow Instructions.
- [ ] Keyframes and interpolation or tracking spans are intentional and reviewable.
- [ ] Dynamic properties start and end on the correct frames.
- [ ] Required ontology values are complete and the item is not Invalid.
- [ ] The explicit workflow action moved the task to the expected next stage.

## Failure modes and recovery

| Symptom | What to check |
|---|---|
| Auto-Tracking drifts or switches identity | Stop tracking, correct the nearest reliable frame, split or shorten the range, and rerun only when the seed and motion are appropriate. |
| Interpolation produces wrong in-between geometry | Add or reposition keyframes around direction, scale, pose, or topology changes; use tracking or manual correction where linear tweening is inappropriate. |
| Track exists on frames where the object is absent | Adjust segment handles or hidden ranges and verify entry/exit policy at frame level. |
| Dynamic property changes at the wrong time | Expand the property row, adjust its range or keyframes, and verify the corresponding object state on boundary frames. |
| Submit is unavailable | Check active stage, assignment, required ontology values, Invalid state, save completion, and workflow configuration. |
...
![The video Workbench coordinates canvas editing, timeline navigation, object state, and review actions in one screen.](../.gitbook/assets/video-workbench.png)

*The video Workbench coordinates canvas editing, timeline navigation, object state, and review actions in one screen.*
